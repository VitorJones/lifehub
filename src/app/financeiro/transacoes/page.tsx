...
export default function TransacoesPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const [ts, cats, cons] = await Promise.all([resT.json(), resC.json(), resCon.json()]);
      setTransacoes(ts);
      setCategorias(cats);
      setContas(cons);
    } finally {
            ...
            <div className="flex rounded-xl border border-[#27272a] overflow-hidden">
              {(["despesa", "receita"] as const).map((t) => (
                <button
                  key={t}
                  onClick={() => {
                    setForm((f) => ({ ...f, tipo: t, categoriaId: "" }));
                  }}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent className="bg-[#1a1a1f] border-[#27272a]">
                    <SelectItem value="__none__" className="text-[#a1a1aa] focus:bg-[#222228]">
                      Nenhuma
                    </SelectItem>
                    {contas.map((c) => (
                      <SelectItem key={c.id} value={c.id} className="text-[#f5f5f5] focus:bg-[#222228]">
              ...
              <div className="col-span-2 flex items-center gap-3">
                <button
                  ...
                  className={`w-10 h-5 rounded-full transition-colors relative ${
                    form.recorrente ? "bg-[#f97316]" : "bg-[#27272a]"
                  }`}
                >
                  <span
                    className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform ${
                      form.recorrente ? "translate-x-5" : "translate-x-0.5"
                    }`}
                  />
                </button>
                <Label className="text-[#a1a1aa] text-sm cursor-pointer">
